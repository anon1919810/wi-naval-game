...
同时平衡** —— 这是二维求根，与 2.1 的一维求 d 不是同一层的事，别混在一起写。
验收：方箱解析解 + 一个已知纵倾的案例；迭代收敛且稳定。
做法提示：`integrate()` 已返回 `xlcb`；外层用两层迭代（内层解 d 使体积达标、
外层解 θ 使 xlcb 对齐 LCG）或 Newton 法，注意给 θ 设合理上下界。
**做完请勾选 2.2 并提交。**
