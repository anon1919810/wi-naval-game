...
run: blender --background HMS_Queen_Mary_1913_Refined_v3.blend --python verify_blender.py
"""
...
def main():
    ...
    qm.CASEMATE_PLACEMENT = objects['Queen_Mary'].get('casemate_placement', 'between_decks')
    hull = objects['Hull']
    pole_ok = all(objects[n].get('legs') == 1 and
                  objects[n].get('component_role') == 'pole_mast'
                  for n in ('Mast_Fore', 'Mast_Main'))
    check('Early-photo pole masts replace both tripods', pole_ok)
    hollow_guns = []
    for key in 'ABQX':
        for side in ('Port', 'Starboard'):
            gun = objects['Barrel_{}_{}'.format(key, side)]
            hit, point, _, _ = gun.ray_cast(Vector((0, qm.BARREL_END_M + .1, 0)), Vector((0, -1, 0)))
            hollow_guns.append(hit and point.y < qm.BARREL_END_M - .5)
    check('Eight muzzle bores have actual recessed geometry', all(hollow_guns))
    hollow_funnels = []
    for i, (_, rx, _) in qm.FUNNELS.items():
        funnel = objects['Funnel_' + str(i)]
        top = qm.FUNNEL_TOP_Z - funnel.location.z
        hit, point, _, _ = funnel.ray_cast(Vector((rx * .45, .37, top + .1)), Vector((0, 0, -1)))
        hollow_funnels.append(hit and point.z < top - .5)
    check('Three funnel mouths have recessed interiors', all(hollow_funnels))
    recesses = []
    for y in qm.CASEMATE_YS:
        for sign in (-1, 1):
            surface = qm.deck_halfbeam(y)
            start = Vector((sign * (surface + .2), y, qm.casemate_axis_z(y)))
            hit, point, _, _ = objects['Forecastle'].ray_cast(start, Vector((-sign, 0, 0)))
            recesses.append(hit and abs(point.x) < surface - .5)
    if qm.CASEMATE_PLACEMENT == 'between_decks':
        check('Sixteen casemate openings are recessed into the forecastle', all(recesses))
    else:
        check('Alternative on-forecastle mode records no below-deck cutouts',
              objects['Forecastle'].get('cutout_count') == 0)
    ...
    supported_roles = ('forecastle_deck', 'quarterdeck', 'bridge', 'conning_tower',
                       'structure', 'funnel', 'pole_mast', 'main_turret', 'barbette',
                       'fire_control')
    ...
    obstruction_roles = ('funnel', 'funnel_uptake', 'bridge', 'conning_tower', 'structure',
                         'pole_mast', 'fire_control', 'forecastle_deck', 'quarterdeck',
    ...
    (OUT / 'firing_arcs.json').write_text(json.dumps({
        ...
        'increment_deg': 5,
        'note': 'Other turrets, barbettes and barrels are included at rest; own mount is excluded. Only port barrel is sampled. Thin visual rigging/rails are excluded. Not a simultaneous turret collision or firing safety model.',
    ...
    for name in ('Mast_Fore', 'Mast_Main'):
        check('{}: pole mast records one main spar'.format(name),
              objects[name].get('legs') == 1,
              source=qm.SOURCES['early_photos'], layer='assumption',
              status='photo_interpretation: exact dimensions and dated fit unverified')
    check('Early pole masts follow the latest user correction; dated fit remains provisional',
          True, source=qm.SOURCES['user'], layer='assumption',
          status='user approved early-photo interpretation, not historical certification')
