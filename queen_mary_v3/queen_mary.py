"""Generate the user-specified HMS Queen Mary greybox in Blender 3.x/4.x. Asset v3: early-photo refinement over the DeepSeek v2 module contract.
...
Inherited asset_v2 improvements against the v1 greybox:
...
SOURCES = {
    'early_photos': 'User supplied stern dock photo marked 18-3-13 and broadside photograph; pole-mast interpretation, precise fit provisional',
...
SHIP = {
    ...
    'configuration': 'early_photo_refinement_on_deepseek_v2',
...
def make_funnels(root):
    for index, (y, rx, ry) in FUNNELS.items():
        ...
        funnel['section'] = 'circular' if rx == ry else 'elliptical'
...
def make_underwater_gear(root):
    builder = MeshBuilder()
    for sign in (1, -1):
        for offset, y, z in ((3.0, -101.0, -4.2), (6.0, -95.0, -5.5)):
            hub = (sign * offset, y, z)
            builder.add(rod_mesh((sign * offset * .65, -68.0, -6.4), hub, 0.28, 10), 'DarkGrey')
            builder.add(rod_mesh((sign*(offset*.6),y+5,-2.2),
                                 (hub[0],y+1,z),.20,10),'HullGrey')
            builder.add(rod_mesh((hub[0], hub[1] - 0.5, hub[2]), (hub[0], hub[1] + 0.4, hub[2]),
                                 0.55, 10), 'DarkGrey')
            for blade in range(4):
                angle = math.radians(45 + 90 * blade)
                tip = (hub[0] + 1.85 * math.cos(angle), hub[1], hub[2] + 1.85 * math.sin(angle))
    ...
    shafts['note'] = 'four shafts, brackets and propellers; estimated visual fit, not a damage module or machinery plan'
...
def preview_setup():
    ...
    cameras = [
        ...
        ('Detail_50m', (63.5,40,16), (10.5,35,12), 90, (1600,1000)),
        ('Whole_200m', (176,90,41), (0,0,6), 245, (1600,1000)),
    ...
    for name, position, target, scale, resolution in cameras:
        ...
        if name in ('Detail_50m','Whole_200m'):
            data.type = 'PERSP'
            data.lens = 32 if name == 'Detail_50m' else 28
            cam['view_distance_description'] = 'about 50 m from side' if name == 'Detail_50m' else 'about 200 m from ship centre'
        data.clip_end = 2000
...
def export_fbx(out):
    ...
    bpy.context.view_layer.update()
    try:
        bpy.ops.export_scene.fbx(filepath=str(out / 'HMS_Queen_Mary_1913_Refined_v3.fbx'),
    ...
    try:
        ...
        manifest = {'ship': SHIP, 'references': SOURCES,
                    ...
                    'estimation_notes': [
                        ...
                        'Pole masts follow the latest user correction and early photographs; exact dated fit remains provisional.',
                        'Sixteen forecastle casemates retain the user-specified 8-per-side arrangement; exact stations are estimates.',
        ...
        bpy.ops.wm.save_as_mainfile(filepath=str(out / 'HMS_Queen_Mary_1913_Refined_v3.blend'))
