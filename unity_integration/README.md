...
资产本体在 `..\queen_mary\`，两边靠 `sync_to_unity.ps1` 同步。
...
```powershell
cd "C:\Users\杨睿\Desktop\HMS_Queen_Mary_建模成果_2026-09-17\unity_integration"
...
cp -f "$SRC/unity_integration/Assets/Scripts/Naval/ShipContract.cs" "$P/Assets/Scripts/Naval/"
cp -f "$SRC/unity_integration/Assets/Editor/ShipModelImportSettings.cs" "$P/Assets/Editor/"
cp -f "$SRC/unity_integration/Assets/Editor/ShipAssetValidator.cs" "$P/Assets/Editor/"
cp -f "$SRC/queen_mary/HMS_Queen_Mary_1913_Greybox.fbx" "$P/Assets/Resources/Ships/HMS_Queen_Mary_1913/"
for f in ship_contract.unity.json object_manifest.json damage_model.json hydrostatics.json \
  ...
  cp -f "$SRC/queen_mary/$f" "$P/Assets/Resources/Ships/HMS_Queen_Mary_1913/"
done
```
...
| `..\queen_mary\*.fbx` `*.json` | `Assets/Resources/Ships/HMS_Queen_Mary_1913/` | FBX ＋ 全部数据文件 |
...
详细报告同时写回 `..\queen_mary\unity_verification.json`（覆盖 v1 那份失效记录）。
...
退出码 0 = 通过、1 = 失败，报告同样写回 `..\queen_mary\unity_verification.json`。
...
```
queen_mary/unity_preview/{Overview,Starboard,Top,Bow,Stern_Aft}.png
queen_mary/unity_render_check.json
```
